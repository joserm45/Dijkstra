...
void j1Map::Path(int x, int y)
{
	...
	int num_current = visited.find(curr);

	while (curr != visited.start->data && num_current != -1)
	{
			curr = breadcrumbs[num_current];
			path.PushBack(curr);
			num_current = visited.find(breadcrumbs[num_current]);
	}
